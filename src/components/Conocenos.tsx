...
export default function Conocenos() {
  return (
    <section id="conocenos" className="py-20">
      <Card className="overflow-hidden shadow-lg" data-aos="zoom-in">
        <CardHeader className="bg-primario text-segundario p-6">
        ...
        <CardContent className="p-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8 ">
            {nosotros.map((service, index) => (
              <div
                key={index}
                className="flex items-start bg-colorfondo p-4 rounded-lg  shadow-md transition-all duration-300 hover:shadow-xl hover:-translate-y-1"
              >
                <div className="flex-shrink-0 mr-4">{service.icon}</div>
                <div>
                  <h3 className="font-semibold text-xl mb-2 text-acento">
                    {service.title}
                  </h3>
